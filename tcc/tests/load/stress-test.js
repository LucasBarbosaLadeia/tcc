...
export const options = {
  scenarios: {
    ...
    recep_scenario: {
      ...
      startVUs:    0,
      stages: [
        { duration: '2m', target: 34 },
        { duration: '6m', target: 34 },
        { duration: '2m', target: 0  },
      ],
      exec: 'recepFlow',
    },
...
export function adminFlow() {
  ...
  if (!token) { errors.add(true); return; }
  ...
  reqs.forEach((res) => check(res, { 'admin req ok': (r) => r.status === 200 }));
  errors.add(false);
  sleep(1.5);
}
...
export function recepFlow() {
  ...
  reqs.forEach((res) => check(res, { 'recep req ok': (r) => r.status === 200 }));
  errors.add(false);
  sleep(1.5);
}
...
export function pacienteFlow() {
  ...
  reqs.forEach((res) => check(res, { 'paciente req ok': (r) => r.status === 200 }));
  errors.add(false);
  sleep(1.5);
}
