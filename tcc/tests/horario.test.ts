...
jest.mock(require.resolve("../Backend/src/models/horarioModel"), () => ({ __esModule: true, default: mockHorario }));
...
describe("Horario Controller", () => {
  ...
  test("Cria horário e valida datas", async () => {
    const respBad = await request(app).post("/api/datas").send({ id_agenda: 1 });
    expect(respBad.status).toBe(400);

    const resp = await request(app).post("/api/datas").send({ id_agenda: 1, data_hora_inicio: new Date(Date.now() + 3600 * 1000), data_hora_fim: new Date(Date.now() + 7200 * 1000) });
  ...
  test("GET available horarios", async () => {
    ...
    const r = await request(app).get("/api/datas");
  ...
  test("PUT/DELETE horário", async () => {
    ...
    const r1 = await request(app).put("/api/datas/1").send({ status: "Indisponível" });
    expect(r1.status).toBe(200);
    const r2 = await request(app).delete("/api/datas/1");
