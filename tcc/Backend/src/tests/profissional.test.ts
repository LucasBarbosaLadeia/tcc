...
jest.mock(require.resolve("../Backend/src/models/profissionalModel"), () => ({ __esModule: true, default: mockProfissional }));
...
describe("Profissional Controller", () => {
  ...
  test("Cria profissional e valida campos obrigatórios", async () => {
    ...
    const bad = await request(app).post("/api/profissionais").send({});
    expect(bad.status).toBe(400);

    const ok = await request(app).post("/api/profissionais").send({ cpf: "1", registro_profissional: "r", tipo_registro: "t", nome_completo: "N", id_especialidade: 1, id_unidade: 1, telefone: "t" });
  ...
  test("GET list e GET by id", async () => {
    ...
    const r1 = await request(app).get("/api/profissionais");
    expect(r1.status).toBe(200);
    const r2 = await request(app).get("/api/profissionais/1");
